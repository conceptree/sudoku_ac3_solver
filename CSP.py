...
class csp:
	...
	def __init__ (self, domain = digits, grid = ""):
		self.variables = squares # Variables 
		self.domain = self.getDict(grid) # Domain
		...
		self.constraints = {(variable, peer) for variable in self.variables for peer in self.peers[variable]} # Constrains
